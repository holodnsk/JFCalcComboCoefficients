...
public class AlbertaHand {
   ...
   public AlbertaHand(String cs) {
      ...
      while(t.hasMoreTokens()) {
         ...
         if (s.length()==2) {
            AlbertaCard c = new AlbertaCard(s.charAt(0),s.charAt(1));
            if (c.getIndex() != AlbertaCard.BAD_CARD)
               addCard(c);
         }
      }     
   }
    ...
    */
   public AlbertaHand(AlbertaHand h) {
      cards = new int[MAX_CARDS + 1];
      cards[0] = h.size();
    ...
    */
   public boolean addCard(AlbertaCard c) {
    ...
    */
   public AlbertaCard getCard(int pos) {
      if (pos < 1 || pos > cards[0]) return null;
      return new AlbertaCard(cards[pos]);
   }
    ...
    * @param c the card to add
    */
   public void setCard(int pos, AlbertaCard c) {
