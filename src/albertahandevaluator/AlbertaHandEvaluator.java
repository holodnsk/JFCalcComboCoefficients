...
public class AlbertaHandEvaluator {
    ...
    */
   public int rankHand(AlbertaCard c1, AlbertaCard c2, AlbertaHand h) {
      h.addCard(c1);
      h.addCard(c2);
      int rank = rankHand(h);
      h.removeCard();
    ...
    */
   public static String nameHand(AlbertaHand h) {
    ...
    */
   public int compareHands(AlbertaHand h1, AlbertaHand h2) {
      int r1 = rankHand(h1);
      int r2 = rankHand(h2);
    ...
    * @return 1 = first hand is best, 2 = second hand is best, 0 = tie
    */
   public int compareHands(int rank1, AlbertaHand h2) {
      int r1 = rank1;
      int r2 = rankHand(h2);
    ...
    */
   public int[][] getRanks(AlbertaHand board) {
      AlbertaHand myhand = new AlbertaHand(board);
      int[][] rc = new int[52][52];
      int i, j, v, n1, n2;
      AlbertaDeck d = new AlbertaDeck();
      d.extractHand(board);

      // tabulate ranks
      for (i = d.getTopCardIndex(); i < AlbertaDeck.NUM_CARDS; i++) {
         myhand.addCard(d.getCard(i));
         n1 = d.getCard(i).getIndex();
         for (j = i + 1; j < AlbertaDeck.NUM_CARDS; j++) {
    ...
    *         input.
    */
   public AlbertaHand getBest5CardHand_holdem(AlbertaHand h) {
      int[] ch = h.getCardArray();
      int[] bh = new int[6];
      int j = Find_Hand(ch, bh);
      AlbertaHand nh = new AlbertaHand();
      for (int i = 0; i < 5; i++)
         nh.addCard(bh[i + 1]);
      return nh;
   }

   // TODO: implement real omaha comparison (with 2-cards from hand restriction)
   public AlbertaHand getBest5CardHand_omaha(AlbertaHand h) {
      int[] ch = h.getCardArray();
      int[] bh = new int[8];
      int j = Find_Hand(ch, bh);
      AlbertaHand nh = new AlbertaHand();
   ...
   private static final int ID_GROUP_SIZE = (AlbertaCard.NUM_RANKS * AlbertaCard.NUM_RANKS
         * AlbertaCard.NUM_RANKS * AlbertaCard.NUM_RANKS * AlbertaCard.NUM_RANKS);

   private final static byte ID_ExistsStraightFlush(AlbertaHand h, byte major_suit) {
      boolean[] present = new boolean[AlbertaCard.NUM_RANKS];
      // for (i=0;i<Card.NUM_RANKS;i++) present[i]=false;

      for (int i = 0; i < h.size(); i++) {
         int cind = h.getCardIndex(i + 1);
         if (AlbertaCard.getSuit(cind) == major_suit) {
            present[AlbertaCard.getRank(cind)] = true;
         }
      }
      int straight = present[AlbertaCard.ACE] ? 1 : 0;
      byte high = 0;
      for (int i = 0; i < AlbertaCard.NUM_RANKS; i++) {
         if (present[i]) {
   ...
   private final static int ID_KickerValue(byte[] paired, int kickers,
         byte[] not_allowed) {
      int i = AlbertaCard.ACE;
      int value = 0;
      while (kickers != 0) {
         while (paired[i] == 0 || i == not_allowed[0] || i == not_allowed[1])
            i--;
         kickers--;
         value += pow(AlbertaCard.NUM_RANKS, kickers) * i;
         i--;
      }
      return value;
   }

   private final static int ID_KickerValueSuited(AlbertaHand h, int kickers, byte suit) {
      int i;
      int value = 0;
    ...
    *         the hand is.
    */
   public final static int rankHand(AlbertaHand h) {
      ...
      byte[] paired = new byte[AlbertaCard.NUM_RANKS];       // array to track paired carsd
      ...
      byte straight_size;
      // flush
      byte[] suit_size = new byte[AlbertaCard.NUM_SUITS];
      byte major_suit = 0;
      ...
      for (r = 0; r < h.size(); r++) {
         ...
         rank = (byte) AlbertaCard.getRank(cind);
         suit = (byte) AlbertaCard.getSuit(cind);
      ...
      straight_size = (byte) (paired[AlbertaCard.ACE] != 0 ? 1 : 0);

      for (int i = 0; i < (POKER_HAND + 1); i++) {
         pair_rank[i][0] = (byte) AlbertaCard.NUM_RANKS;
         pair_rank[i][1] = (byte) AlbertaCard.NUM_RANKS;
      }

      // check for straight and pair data
      // Start at the Deuce.  straight_size = 1 if we have an ace.
      for (r = 0; r < AlbertaCard.NUM_RANKS; r++) {
         // check straight
         if (paired[r] != 0) {
      ...
      if (group_size[4] != 0) {
         ...
         ident += pair_rank[4][0] * AlbertaCard.NUM_RANKS;
         pair_rank[4][1] = (byte) AlbertaCard.NUM_RANKS; // just in case 2 sets quads
         ident += ID_KickerValue(paired, 1, pair_rank[4]);
      } else if (group_size[3] >= 2) {
         ident = FULLHOUSE * ID_GROUP_SIZE;
         ident += pair_rank[3][0] * AlbertaCard.NUM_RANKS;
         ident += pair_rank[3][1];
   ...
   public static void main(String args[]){
      AlbertaHand albertaHand1 = new AlbertaHand("Ad Kd Qd Jd Td");
      AlbertaHand albertaHand2 = new AlbertaHand("Kc Qc Jc Tc 9c");

      AlbertaHandEvaluator handEval = new AlbertaHandEvaluator();

      System.out.println(handEval.getBest5CardHand_holdem(albertaHand1));
      System.out.println(handEval.getBest5CardHand_holdem(albertaHand2));

      System.out.println(handEval.rankHand(albertaHand1));
      System.out.println(handEval.rankHand(albertaHand2));

      System.out.println(handEval.nameHand(albertaHand1));
      System.out.println(handEval.nameHand(albertaHand2));
